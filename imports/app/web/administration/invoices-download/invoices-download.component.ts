...
export class InvoicesDownloadPage implements OnInit, OnDestroy {
    ...
    /**
     * This function update invoices history by month and year
     */
    monthChange() {
        ...
        this._invoices = Invoices.find({
            restaurant_id : this._selectedRestaurantId,
            creation_date: {
    ...
    invoiceGenerate( _pInvoice : Invoice, _pCountryId: string ) {
        if( !Meteor.userId() ){
            let titleMsg = this.itemNameTraduction('INVOICES_DOWNLOAD.SYSTEM_MSG');
            let btnAcceptLbl = this.itemNameTraduction('CLOSE');
            var error : string = 'LOGIN_SYSTEM_OPERATIONS_MSG';
            this.openDialog(titleMsg, '', error, '', btnAcceptLbl, false);
            return;
        }
